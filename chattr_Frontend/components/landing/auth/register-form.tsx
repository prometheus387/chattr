...
export function RegisterForm() {
  ...
  // If the user is already authenticated, skip the form entirely.
  useEffect(() => {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      await auth.register({
        ...
        securityQuestion: values.question,
        securityAnswer: values.answer.trim(),
      });
      setSuccess(true);
    } catch (err) {
      if (err instanceof ApiError) {
        if (err.status === 409) {
          ...
          // Backend validation message — surface it as a form-level error.
